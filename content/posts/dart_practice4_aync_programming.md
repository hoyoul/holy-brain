...
## [1- future] 비동기가 필요한 상황 simulation (동기화 코드) {#1-future-비동기가-필요한-상황-simulation--동기화-코드}
...
## [2- future] 비동기가 필요한 상황 simulation 해결(비동기 코드) {#2-future-비동기가-필요한-상황-simulation-해결--비동기-코드}
...
## [3- future] 비동기 코드의 문제점과 해결 (await &amp; async) {#3-future-비동기-코드의-문제점과-해결--await-and-async}
...
## [4- future] 비동기로 실행되는 함수의 문제점 {#4-future-비동기로-실행되는-함수의-문제점}
...
## [5- future] 비동기로 실행되는 함수의 문제점 해결 {#5-future-비동기로-실행되는-함수의-문제점-해결}
...
## [6-stream] stream의 개념 {#6-stream-stream의-개념}
...
## [7-stream] 간단한 stream의 예. {#7-stream-간단한-stream의-예-dot}
...
## [8-stream] 여러명의 구독자(관찰자,stream)이 있는 경우 {#8-stream-여러명의-구독자--관찰자-stream--이-있는-경우}
...
## [9-stream] 2개의 streamlistener에서 하나는 짝수만, 하나는 홀수 출력 {#9-stream-2개의-streamlistener에서-하나는-짝수만-하나는-홀수-출력}
...
## [10-stream] yield simulation. {#10-stream-yield-simulation-dot}
...
## [11-stream] for-loop를 stream으로 만들기. {#11-stream-for-loop를-stream으로-만들기-dot}
